...
- Go to Images → AMIs.
- Search for your AMI by its name or ID. Verify the status is available.
- Confirm that the AMI's status is available.
...
This lists all your AMIs, showing their IDs, names, and current status. Ensure the State is available.

### Delete an AMI if no longer needed:
Console: Select the AMI → Deregister.

CLI:
aws ec2 deregister-image --image-id <ami-id>

### Other methords for building AMI
- Ansible 
- AWS CLI 
- AWS Console 
